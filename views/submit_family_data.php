...
if ($_POST) {
    ...
    if (!empty($family_passport_number)) {
        $family_passport_number = implode(",",$family_passport_number);
    }

    if (!empty($family_passport_issue_date)) {
        $family_passport_issue_date = implode(",",$family_passport_issue_date);
    }

    if (!empty($family_passport_expire_date)) {
        $family_passport_expire_date = implode(",",$family_passport_expire_date);
    }
    ...
    $value = array($inquiry_id, $applicant_name, $relationship, date("Y-m-d", strtotime($marriage_date)), $familyAddress, date("Y-m-d", strtotime($date_of_birth)),
        $family_passport_number, $family_passport_issue_date, $family_passport_expire_date, $migrate_with_client, $family_name, $given_name, $reason_for_change, $currentDate, 1);
    ...
    if ($id) {
        ...
        echo json_encode(array('status' => true, 'id' => $id, 'applicant_name' => $applicant_name, 'relation' => $raltionName['name'],
            'address' => $familyAddress, 'date_of_birth' => date("d/m/Y",strtotime($date_of_birth)),"reason_for_change" =>$reason_for_change,
            'link' => $link,'data' => $arrData));
    } else {
